...
# ============================================================================
```{r}
# Create directories for better workflow
...
seurat_list <- lapply(sample_dirs, function(sdir) {
  ...
  obj <- CreateSeuratObject(            # Create seurat object for each sample
    counts = counts,
    project = "GSE279086",
    min.cells = 3, # keep features that are expressed in atleast 3 cells
    min.features = 200 # keep cells that express atleast 200 genes
  )
...
length(seurat_list) # Check if length is same as number of sample dir or samples
```
